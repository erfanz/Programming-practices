...
private:
	bool GetOneFeature(const Point& point, Feature* feature) {
		// We can optionally set RPC configuration values on context object, e.g. deadlines
		// Note that you cannot reuse this object between calls
		ClientContext context;
		
		// We pass the context, request, and response to the stub object
...
int main(int argc, char** argv) {
	// Expect only arg: --db_path=path/to/route_guide.db.json
	std::string db = routeguide::GetDbFileContent(argc, argv);
	RouteGuideClient guide(
		grpc::CreateChannel("localhost:50051", grpc::InsecureChannelCredentials()), db);
